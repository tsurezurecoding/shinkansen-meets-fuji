...
const REFERENCES = {
  ...
  shizuokaFrostFan: {
    label: { ja: "静岡県: 茶畑の防霜ファンは何のため？", en: "Shizuoka Prefecture: Why tea fields use frost-protection fans (Japanese only)" },
    url: "https://opendata.pref.shizuoka.jp/dataset/6070/resource/42712/shimadakenteians_vol1young.pdf",
  },
  naroFrostFan: {
    label: { ja: "農研機構: 茶園用防霜ファンのしくみ", en: "NARO: How frost-protection fans work in tea fields (Japanese only)" },
    url: "https://www.naro.go.jp/project/results/laboratory/vegetea/2009/vegetea09-18.html",
  },
  tsumagoiResort: {
...
const SPOTS = [
  ...
  },
  {
    ...
    minutesFromTokyo: 10, side: "E", category: "notable", confidence: "verified", durationSec: 15, spotting: "moderate", scene: "fuji",
  ...
  },
  {
    id: "fuji-bus-sales",
    icon: "🚌",
    ja: { name: "富士バス販売", area: "三島 → 新富士（富士市川成島）", hook: "窓の下に、バスの大行列。", story: "E席側で富士山が大きく見えるころ、反対のA席側には何列ものバス。ここは中古バスの売買を手がける富士バス販売です。観光、路線、送迎――別々の道を走ってきた車両が、次の仕事を待つ場所。新富士手前の数秒に、バスの『次の出発』が見えてきます。" },
    en: { name: "Fuji Bus Sales", area: "Mishima → Shin-Fuji, Fuji City", hook: "Rows and rows of buses.", story: "As Mt. Fuji looms on the Seat E side, rows of buses appear across the train on Seat A. This is Fuji Bus Sales, a used-bus dealer. Coaches, route buses and shuttles built for different jobs gather here while they wait for their next one. In the few seconds before Shin-Fuji, you catch a glimpse of buses between journeys." },
    pageTitle: { ja: "新幹線から見える富士バス販売｜新富士手前のA席に並ぶ中古バス | 新幹線の窓", en: "Fuji Bus Sales from the Shinkansen | Rows of Buses near Shin-Fuji" },
    pageHeading: { ja: "富士山の反対側に、バスの大行列", en: "Rows of buses across from Mt. Fuji" },
    pageHeadingChunks: { ja: ["富士山の反対側に、", "バスの大行列"], en: ["Rows of buses", "across from Mt. Fuji"] },
    metaDescription: { ja: "新富士駅近くの新幹線A席側に見える富士バス販売。中古バスが何列も並ぶ展示場を、車窓写真・動画・地図とともに紹介。曇って富士山が見えない日にも楽しめます。", en: "Near Shin-Fuji, Seat A looks down on rows of used buses at Fuji Bus Sales. See the yard in window photos and a video, even on a cloudy Mt. Fuji day." },
    sectionHeading: { ja: "あの大量のバスは何？", en: "Why are so many buses parked here?" },
    pageStory: { ja: "バスはいつも、どこかへ人を運んでいます。ところが新富士の手前では、観光バスも路線バスも送迎バスも、一か所に集まって静かに並ぶ。A席側に見える富士バス販売は、中古バスを買い取り、次の使い手へ売る専門店です。ある会社での仕事を終えた車両が、別の土地や別の用途へ向かう途中にいる。窓を横切る大行列は、バスたちの終点ではなく、次の出発を待つ景色なのです。", en: "Buses usually pass us on their way to somewhere. Just before Shin-Fuji, coaches, route buses and shuttles instead gather in rows on the Seat A side. Fuji Bus Sales buys used buses and offers them to their next operators. A vehicle may have finished one job, but it can be headed for another route, another company or another place. The extraordinary line-up outside the window is a pause between journeys, rather than the end of them." },
    explainer: {
      heading: { ja: "次の仕事を待つバスたち", en: "Buses waiting for their next job" },
      ja: [
        "同じ『バス』でも、仕事が違えばつくりも違います。観光用は座席が多く、路線用には乗り降りしやすいノンステップ車がある。送迎用や小型車も含め、富士バス販売の在庫一覧は用途と大きさで分かれ、年式・定員・走行距離まで載っています。車窓で車体の長さや窓、扉の位置に目を向けると、どんな人を乗せて走ってきたのだろうと想像が広がります。個々の車両の経歴までは、外見だけでは分かりません。",
        "なぜ中古バスを買うのでしょう。新しい路線や送迎を始めるには車両が要りますが、新車をそろえるには大きな初期費用がかかります。国土交通省の自治体調査には、初期投資を抑えるため中古車両を購入し、後に修理費がかさんだという回答もあります。購入価格だけでなく、次の運行で使える状態に保つことまで考えて選ぶ。その現実が、あのバスの列の向こうにあります。",
        "中古バスの次の仕事には、車両を売買するだけでは足りない場合もあります。同社が案内する富士バスボデーは、整備・点検に加え、床や座席の張り替え、外観の塗装や改装も手がけています。実際にどの車両がどの作業を受けたかは分かりませんが、使い手が変われば姿や設備も変わり得る。停まっているバスの向こうに、次の路線や旅が続いています。",
        "公式サイトは在庫を自社に置くと案内しており、新幹線から見えるのはその展示場の一角です。普段は街角で一台ずつ出会うバスが、ここでは何列も一度に現れる。E席側の富士山を追う時間に、A席側ではもう一つの富士市の景色が数秒だけ広がります。山が雲に隠れた日も見つけられる、車窓の小さな驚きです。",
      ],
      en: [
        "The word 'bus' covers very different jobs. Coaches carry seated travellers; some route buses have low floors for easier boarding. Fuji Bus Sales sorts its stock by size and purpose, from sightseeing and route buses to shuttles and minibuses, with each listing giving the age, capacity and mileage. From the train, differences in length, windows and doors invite you to imagine the work these vehicles once did. Their individual histories cannot be read from their appearance alone.",
        "Why buy a used bus? Starting a route or shuttle service takes vehicles, and buying them new means a substantial initial outlay. In a Japanese transport ministry survey, a municipality reported buying used vehicles to reduce its initial investment, then facing higher repair costs. Choosing a bus means considering both the purchase and the work needed to keep it on the road. That practical calculation lies behind the remarkable rows outside the window.",
        "Finding a new operator can involve more than a sale. Fuji Bus Body, promoted on the dealer's website, offers inspections and maintenance as well as new flooring, seat upholstery, repainting and conversions. We cannot tell which vehicles in the yard have had any particular work done. Still, the services show how a bus can change as it prepares for a new role.",
        "The dealer says it keeps its stock on site; the train passes one part of that display yard. Buses that we usually meet one at a time on the road suddenly appear in rows. While passengers on Seat E follow Mt. Fuji, Seat A offers a few seconds of another Fuji City scene, one that remains visible even when clouds hide the mountain.",
      ],
    },
    guideHighlight: { ja: "新富士の手前、A席側の高架下を見てください。「富士バス販売」の赤い看板と白いバスの列が目印です。", en: "Before Shin-Fuji, watch below the Seat A window for the red Fuji Bus Sales sign and rows of white buses." },
    minutesFromTokyo: 44.8, side: "A", category: "notable", confidence: "verified", durationSec: 8, spotting: "easy", scene: "hills", visibleWhenCloudy: true,
    image: "images/20260919_fuji_bus_sales_michikusa.jpg",
    photoCredit: { ja: "michikusa", en: "michikusa", date: "2026-09-19", note: { ja: "第三展示場の看板と並ぶバス", en: "The Third Display Yard sign above rows of buses" } },
    photos: [{ src: "images/20260919_fuji_bus_sales_2_michikusa.jpg", alt: { ja: "富士バス販売の展示場に並ぶ白い中古バス", en: "Rows of white buses at the Fuji Bus Sales display yard" }, credit: { ja: "michikusa", en: "michikusa" }, date: "2026-09-19", note: { ja: "新幹線から見下ろすバスの列", en: "Bus rows viewed from the Shinkansen" } }],
    media: { heading: { ja: "田子の浦港からバスの列まで", en: "From Tagonoura Port to the bus yard" }, description: { ja: "A席側の車窓が田子の浦港から富士バス販売へ移る様子を動画で見られます。", en: "Follow the Seat A window view from Tagonoura Port to Fuji Bus Sales." }, videos: [{ kind: "youtube", id: "sD61YI0y12o", url: "https://www.youtube.com/watch?v=sD61YI0y12o", title: { ja: "田子の浦港から富士バス販売までのA席車窓", en: "Seat A window from Tagonoura Port to Fuji Bus Sales" } }] },
    relatedSpotIds: ["fuji", "fuji-pipe-sign", "fuji-paper-mills"],
    references: [{ label: { ja: "富士バス販売：会社概要", en: "Fuji Bus Sales: company profile (Japanese)" }, url: "https://www.fujibus-sales.co.jp/company.php" }, { label: { ja: "富士バス販売：取扱車両", en: "Fuji Bus Sales: types of buses (Japanese)" }, url: "https://www.fujibus-sales.co.jp/" }, { label: { ja: "富士バスボデー：修理・改装の実績", en: "Fuji Bus Body: repair and conversion work (Japanese)" }, url: "https://www.fujibus-sales.co.jp/body/works.html" }, { label: { ja: "国土交通省：市区町村における公共交通機関に関する調査", en: "Japan transport ministry: municipal public transport survey (Japanese PDF)" }, url: "https://www.mlit.go.jp/common/001007541.pdf" }, { label: { ja: "Merkmal：中古バス市場を考える", en: "Merkmal: the used-bus market (Japanese)" }, url: "https://merkmal-biz.jp/post/77175" }],
    map: { lat: 35.1432, lng: 138.6750, ja: "富士バス販売の展示場付近", en: "Fuji Bus Sales display yard area" },
    viewpoint: { lat: 35.1439, lng: 138.6748 },
  },
  {
    id: "fuji-paper-mills",
    icon: "🏭",
  ...
  {
    id: "sapporo-shizuoka-factory",
    icon: "🏭",
    ja: { name: "サッポロビール静岡工場", area: "静岡 → 掛川（焼津付近）", hook: "銀のタンクが、窓に並ぶ。", story: "静岡を出て焼津へ向かうと、A席側に銀色のタンクが一列に迫り、奥にSAPPOROの星印が見えます。札幌の名を持つサッポロビールが、1980年から焼津でビールを造ってきた工場です。清洲城近くのキリンビール名古屋工場に負けない、ほんの数秒の工業車窓です。" },
    en: { name: "Sapporo Beer Shizuoka Brewery", area: "Shizuoka → Kakegawa, near Yaizu", hook: "A row of silver tanks flashes by.", story: "West of Shizuoka, a row of silver tanks fills the Seat A window, with the SAPPORO star behind it. Despite the northern name, Sapporo Beer has brewed in Yaizu since 1980. Like the Kirin brewery near Kiyosu Castle, this is an industrial view worth catching in the few seconds it lasts." },
    pageTitle: { ja: "新幹線から見えるサッポロビール静岡工場｜SAPPOROの星と焼津のタンク | 新幹線の窓", en: "SAPPORO Beer Shizuoka Brewery from the Shinkansen | Tanks near Yaizu" },
    pageHeading: { ja: "焼津の車窓に迫る、サッポロビールのタンク", en: "Sapporo Beer's tanks beside the tracks near Yaizu" },
    pageHeadingChunks: { ja: ["焼津の車窓に迫る、", "サッポロビールのタンク"], en: ["Sapporo Beer's tanks", "beside the tracks near Yaizu"] },
    metaDescription: { ja: "静岡駅を出て焼津へ向かう新幹線のA席側に、サッポロビール静岡工場のタンク列とSAPPOROの星が現れる。1980年から続く焼津の醸造と静岡麦酒の話も紹介。", en: "Spot the SAPPORO star and silver tank row of Sapporo Beer's Shizuoka brewery from Seat A near Yaizu. Discover why the brewery has been here since 1980." },
    sectionHeading: { ja: "SAPPOROの星とタンク、ここは何の工場？", en: "What is behind the SAPPORO star and tanks?" },
    pageStory: { ja: "静岡駅から西へ進み、焼津市浜当目でA席側を見てください。銀色のタンクが線路近くに連なり、その奥にSAPPOROの星印が現れます。2026年9月4日の下り列車から撮った写真では、タンク列が窓を横切りました。一本ずつの用途は車窓だけでは分かりませんが、サッポロビールは静岡工場に巨大な発酵・熟成用タンクが並ぶと紹介しています。", en: "West of Shizuoka Station, watch Seat A near Hamatome in Yaizu. A silver tank row stands close to the line, with the SAPPORO star behind it. Our westbound photograph from 4 September 2026 catches the tanks as they sweep across the window. You cannot identify each tank's job from the train, but Sapporo Beer describes large fermentation and maturation tanks at this brewery." },
    explainer: {
      heading: { ja: "なぜサッポロビールが焼津に？", en: "Why is Sapporo Beer in Yaizu?" },
      ja: [
        "サッポロビール静岡工場は、焼津市浜当目にあります。北海道で生まれた名を持ちながら、静岡での生産は1980年に始まりました。同社の取材記事では、当時の需要の伸びに対応し、輸送の便と用水事情に恵まれた静岡が工場の場所に選ばれたと説明しています。いまでは研究・開発部門も集まり、単なる出荷拠点ではありません。",
        "窓を埋める銀色の列は、ビールづくりを支えるタンク群です。サッポロビールはこの工場に発酵・熟成用の巨大なタンクが並ぶと紹介しています。どの一本に何が入っているかは外から断定できません。それでも、星印の手前にこれだけの設備が集まる景色は、製品が生まれる場所の大きさを一瞬で伝えます。",
        "ここで造られる「静岡麦酒」は、静岡の人と食に向けて企画された地域限定のビール。2013年に発売され、同社は静岡工場での製造を明記しています。清洲城の近くで見えるキリンビール名古屋工場は有名な車窓ですが、焼津にももう一つのビール工場があります。A席から星とタンクを見つけたら、場所は焼津のサッポロビールです。",
      ],
      en: [
        "Sapporo Beer's Shizuoka brewery is in Hamatome, Yaizu. Despite a name born in Hokkaido, brewing here began in 1980. The company's account says Shizuoka was chosen as beer demand grew because transport links and water were favourable. Research and product development now have a presence here as well: it is more than a dispatch point.",
        "The silver row is part of the equipment behind the beer. Sapporo Beer describes large fermentation and maturation tanks at this site. The train window cannot tell you what is inside any particular tank, but the scale of the row beneath the star gives a striking glimpse of the work behind a familiar brand.",
        "Shizuoka Bakushu, a beer developed for the region, is also brewed here. Launched in 2013, it ties a nationally known name to local food and drink. The Kirin brewery near Kiyosu Castle may be the better-known train-window brewery; Yaizu offers a second, equally immediate view. Watch for the star and the tank row from Seat A.",
      ],
    },
    ...
    references: [
      { label: { ja: "サッポロビール：製造所所在地", en: "Sapporo Beer: brewery location (Japanese)" }, url: "https://www.sapporobeer.jp/info/0000011924/" },
      { label: { ja: "サッポロビール：静岡工場の歩みと静岡麦酒", en: "Sapporo Beer: the Shizuoka brewery and local beer (Japanese)" }, url: "https://www.sapporobeer.jp/shizuokabakusyu/love/" },
      { label: { ja: "サッポロビール公式メディア：静岡に工場を置いた理由", en: "Sapporo Beer: why the brewery is in Shizuoka (Japanese)" }, url: "https://blog.sapporobeer.jp/knowledge/21587/" },
      { label: { ja: "サッポロビール：静岡工場の醸造現場", en: "Sapporo Beer: brewing at Shizuoka (Japanese)" }, url: "https://www.sapporobeer.jp/mugitohop/interview/" },
    ],
    map: { lat: 34.88335, lng: 138.32474, ja: "サッポロビール静岡工場", en: "Sapporo Beer Shizuoka Brewery" },
    viewpoint: { lat: 34.88386648, lng: 138.32258291 },
  },
  {
    ...
    explainer: {
      ...
      en: [
        "The large fans standing above the rows are not wind turbines, and they are not there to cool the leaves in summer. On clear, still nights in early spring, air close to the ground can become colder while relatively warmer air remains a few metres above it. Each frost-protection fan blows that warmer air down toward the young tea buds and mixes the layers, helping to prevent late-frost damage. That is why the fans sit high on poles and point diagonally down at the plants.",
    ...
    guideHighlight: {
      ja: "静岡を過ぎたら、まず線路脇の緑の縞模様に注目してください。次に、畝の上で斜め下を向く羽根を探すと、防霜ファンが一つ見つかったあとに何本も並んでいることに気づきます。東京から大阪へ向かうA席側では、掛川手前でつま恋リゾート彩の郷の看板が畑の奥に見えます。掛川駅前の掛川城とあわせて、「茶畑と扇風機、その次に城」というリズムで覚えると、この区間を見つけやすくなります。",
      en: "After Shizuoka, first notice the striped green pattern beside the tracks. Then look for blades angled down toward the rows: once you spot one frost-protection fan, you will start seeing them all across the fields. Toward Osaka, the Tsumagoi Resort Sainosato sign is visible beyond the fields on Seat A shortly before Kakegawa. Remember the sequence — tea rows and fans, then Kakegawa Castle — and this stretch becomes much easier to catch from the window.",
    },
  ...
  {
    id: "rakusai-egg-tanks",
    icon: "🏭",
    ja: { name: "洛西浄化センターの卵形タンク", area: "京都 → 新大阪（長岡京付近）", hook: "窓の先に、巨大なたまご。", story: "京都を出てA席側を眺めていると、たまねぎ型の巨大な構造物が二つ現れ、数秒で流れ去ります。長岡京市の洛西浄化センターにある「卵形汚泥消化タンク」です。街の下水をきれいにした後に残る汚泥を処理する設備で、あの不思議な形にも理由があります。" },
    en: { name: "Rakusai's Egg-Shaped Digestion Tanks", area: "Kyoto → Shin-Osaka, near Nagaokakyo", hook: "Two giant eggs beside the tracks.", story: "After Kyoto, two giant onion-shaped structures appear at the Seat A window and vanish within seconds. They are egg-shaped sludge digestion tanks at Rakusai's wastewater treatment centre in Nagaokakyo. They process what remains after wastewater is cleaned, and their unusual shape has a practical purpose." },
    pageTitle: { ja: "新幹線から見える洛西浄化センターの卵形汚泥消化タンク | 新幹線の窓", en: "Rakusai's Egg-Shaped Sludge Digestion Tanks from the Shinkansen" },
    pageHeading: { ja: "洛西浄化センターの卵形汚泥消化タンク", en: "Rakusai's egg-shaped sludge digestion tanks" },
    pageHeadingChunks: { ja: ["洛西浄化センターの", "卵形汚泥消化タンク"], en: ["Rakusai's egg-shaped", "sludge digestion tanks"] },
    metaDescription: { ja: "京都から新大阪へ向かうA席側に見える、洛西浄化センターのたまねぎ型の巨大構造物。その正体「卵形汚泥消化タンク」と、汚泥を減らす仕組みを紹介。", en: "Two giant onion-shaped tanks appear from Seat A after Kyoto. Discover the egg-shaped sludge digestion tanks at Rakusai and why wastewater treatment needs them." },
    sectionHeading: { ja: "たまねぎ型の巨大タンク、何をしている？", en: "What do the onion-shaped tanks do?" },
    pageStory: { ja: "京都駅を出て新大阪へ向かい、長岡京市に入るころ、A席側の低い建物の上にふくらんだ輪郭が二つ現れます。たまねぎ型にも、巨大なたまごにも見える正体は、洛西浄化センターの卵形汚泥消化タンク。京都府の施設仕様では直径24m・高さ44m・有効容量9,600m³の槽が二つあります。写真は2026年9月19日の下り列車から撮影しました。", en: "Heading from Kyoto toward Shin-Osaka, look above the lower buildings on Seat A near Nagaokakyo. Two shapes resembling giant onions or eggs suddenly rise into view. Kyoto Prefecture's specification identifies them as egg-shaped sludge digestion tanks, each 24 metres across, 44 metres high and with a usable capacity of 9,600 cubic metres. Our photograph was taken from a westbound train on 19 September 2026." },
    explainer: {
      heading: { ja: "街の水をきれいにした、その先の仕事", en: "What happens after the water is cleaned?" },
      ja: [
        "洛西浄化センターは、京都市・向日市・長岡京市・大山崎町にまたがる桂川右岸の下水を処理する京都府の施設です。下水の汚れを微生物が取り込むと、その微生物などが汚泥として残ります。卵形タンクの中では酸素を使わない微生物の働きで汚泥の有機分を分解し、量を減らすとともに消化ガスを生み出します。車窓の巨大な二基は、水をきれいにした後の工程を担っています。",
        "なぜ卵形なのか。内側が滑らかな曲面なので、底に砂や汚泥がたまりにくく、混ぜやすい形です。京都府の仕様には上下方向にかき混ぜる攪拌機と汚泥循環ポンプが記載されています。外から見える丸い輪郭は飾りではなく、中身を動かし続けるための設計です。卵形タンク一般の仕組みは京都市の解説でも確認できますが、車窓の二基は京都府の洛西浄化センターの設備です。",
        "この施設の役割はタンクだけでは終わりません。京都府は、処理施設の上を「アクアパルコ洛西」という公園にし、きれいにした水を長岡京市の勝竜寺城公園でも再利用していると紹介しています。数秒だけ見えるたまねぎ型の背後には、街の水を戻し、資源を使い直す大きな仕組みがあります。",
      ],
      en: [
        "Rakusai is a Kyoto Prefecture treatment centre serving communities on the right bank of the Katsura River, including parts of Kyoto, Muko, Nagaokakyo and Oyamazaki. Microbes help clean the wastewater, leaving sludge behind. Inside these tanks, other microbes break down its organic matter without oxygen, reducing the sludge and producing digestion gas. The two enormous shapes are part of the work that continues after the water looks clean.",
        "Why an egg? Smooth curved walls leave fewer places for sand and sludge to settle and make the contents easier to mix. Kyoto Prefecture's specification lists vertical screw mixers and sludge circulation pumps here. The rounded outline is engineering, not decoration. Kyoto City's explanation describes how egg-shaped tanks work in general; the pair in this view belongs to Kyoto Prefecture's Rakusai centre.",
        "The wider site has another life above ground. Kyoto Prefecture says the space over its treatment facilities is used as Aqua Parco Rakusai, while treated water is reused at Shoryuji Castle Park in Nagaokakyo. The giant onions that flash past the window belong to a much larger system for cleaning water and using resources again.",
      ],
    },
    ...
    references: [
      { label: { ja: "京都府：洛西浄化センター施設仕様", en: "Kyoto Prefecture: Rakusai facility specification (Japanese)" }, url: "https://www.pref.kyoto.jp/ryuiki/news/documents/07kyoutsushiyousyo.pdf" },
      { label: { ja: "京都府：桂川右岸流域下水道と洛西浄化センター", en: "Kyoto Prefecture: Rakusai and the Katsura River sewer system (Japanese)" }, url: "https://www.pref.kyoto.jp/ryuiki/ugan-rakusai.html" },
...
// 特集ページの見出しは施設名だけだが、タイムラインでは他のスポットと並ぶので
// 「何が見えるのか」を名前に入れる。既に観覧車と分かる名前はそのまま使う。
function wheelDisplayName(name, lang) {
  if (lang === "ja") return /観覧車/.test(name) ? name : `${name}の観覧車`;
  return /wheel/i.test(name) ? name : `${name} Ferris Wheel`;
}
