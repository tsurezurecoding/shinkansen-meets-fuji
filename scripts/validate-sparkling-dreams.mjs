...
const japaneseStart = await readAppFile("start.html");
const englishTop = await readAppFile("en/index.html");
const englishStart = await readAppFile("en/start.html");
...
expect(!japaneseStart.includes('class="seasonal-entry"'), "start.html: standalone upper seasonal entry must be absent");
expect(!englishTop.includes('class="seasonal-entry"'), "en/index.html: standalone upper seasonal entry must be absent");
expect(japaneseStart.includes('data-cta-id="top_journey_disney"') && japaneseStart.includes('data-cta-id="top_footer_disney"') && japaneseStart.includes('href="sparkling-dreams.html"'), "start.html: journey and mobile Disney promo cards must remain");
expect(englishStart.includes('data-cta-id="top_journey_disney"') && englishStart.includes('data-cta-id="top_footer_disney"') && englishStart.includes('href="en/sparkling-dreams.html"'), "en/start.html: journey and mobile Disney promo cards must remain");
expect(japaneseStart.includes('data-cta-id="top_journey_727"') && japaneseStart.includes('data-cta-id="top_footer_727"') && japaneseStart.includes('href="727-collection.html"'), "start.html: journey and mobile 727 Collection promo cards must remain");
expect(englishStart.includes('data-cta-id="top_journey_727"') && englishStart.includes('data-cta-id="top_footer_727"') && englishStart.includes('href="en/727-collection.html"'), "en/start.html: English 727 Collection links must point at the English collection page");
...
expect(stylesheet.includes(".sd-post-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 18px; width: min(100%, 760px); margin: 18px auto 0; }"), "style.css: the video grid must stay centred under its section heading");
expect(stylesheet.includes(".sd-post-embed { display: grid; grid-template-rows: auto 1fr;"), "style.css: cards must equalise height per row so credit rows line up");
expect(stylesheet.includes(".sd-x-embed.is-loading .sd-media-frame::after") && stylesheet.includes("sd-embed-shimmer"), "style.css: loading skeleton must be opt-in");
expect(/\.sd-media-frame > blockquote\.twitter-tweet \{[^}]*visibility: hidden;/.test(stylesheet), "style.css: the raw X blockquote text must never lay out before widgets.js renders");
expect(stylesheet.includes(".sd-media-frame > .twitter-tweet-rendered") && stylesheet.includes(".sd-media-frame::after { content: attr(data-placeholder);"), "style.css: rendered X widget reveal or the behind-the-player source label is missing");
expect(!stylesheet.includes("`r`n"), "style.css: literal PowerShell newline escapes leaked into the stylesheet");
...
expect(!/\.sd-post-embed\.sd-x-embed/.test(stylesheet) && !stylesheet.includes(".has-widget") && (stylesheet.match(/\.sd-media-frame \{/g) || []).length === 1, "style.css: a duplicate Disney card layer was appended instead of editing the single card contract");
...
expect(englishPage.includes("Disney Shinkansen") && englishPage.includes("Sparkling Dreams Shinkansen") && englishPage.includes("estimated passing times") && englishPage.includes("bullet train"), "en/sparkling-dreams.html: English SEO target is missing");
...
expect(!/<\/span><span class="copy-chunk">/.test(englishPage), "en/sparkling-dreams.html: adjacent copy chunks must be separated by a space or English words run together");
...
expect((englishPage.match(/class="twitter-tweet"/g) || []).length === 12 && (englishPage.match(/www\.youtube-nocookie\.com\/embed\//g) || []).length === 2, "en/sparkling-dreams.html: approved embed set changed");
expect((englishPage.match(/<div class="sd-media-frame sd-x-frame" data-placeholder="(?:Video|Post) on X"><blockquote class="twitter-tweet"/g) || []).length === 12 && (englishPage.match(/<div class="sd-media-frame sd-youtube-frame" data-placeholder="[^"]+">/g) || []).length === 2, "en/sparkling-dreams.html: every English embed must use the shared media frame");
